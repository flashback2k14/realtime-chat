window
		.addEventListener(
				...
				function() {
					// Base URL
					var BASEURL = window.location.hostname.indexOf("herokuapp") > 0 ? "https://vertx-realtime-chat.herokuapp.com"
							: "http://localhost:7070";
					// UI Elements
					var txtChatId = document.querySelector("#txtChatId");
					var txtChatName = document.querySelector("#txtChatName");
					var txtChatMessage = document
							.querySelector("#txtChatMessage");
					var ulChatHistory = document
							.querySelector("#ulChatHistory");
					var pErrorMessage = document
							.querySelector("#pErrorMessage");
							...
							eventBus
									.registerHandler(
											...
											function(error, message) {
												...
												_createListItem(JSON
														.parse(message.body).author
														+ " says "
														+ JSON
					...
					function _sendChatMessage() {
						...
						xhr.open("POST", BASEURL + "/api/chats/"
								+ txtChatId.value + "/messages")
						xhr
					...
					function init() {
						...
						btnSend.addEventListener("click", _sendChatMessage,
								false);
					}
					;
